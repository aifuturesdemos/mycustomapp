...
def get_paddle_speed():
    try:
        ...
        # Strictly check for positive integer and reasonable range
        if user_input.isdigit():
            paddle_speed = int(user_input)
            if 1 <= paddle_speed <= 20:
                return paddle_speed
            else:
                print("Paddle speed out of allowed range (1-20). Using default.")
        else:
            print("Invalid input: Only positive integers are allowed. Using default.")
    except (IndexError, ValueError):
        print("No valid command-line input provided. Using default.")
